# 👨‍💻 Mateus Pereira Barboza  
**`Analista de Sistemas & Dados | BI | Automação | Integrações`**

🚀 **Transformando dados, sistemas e processos em decisões inteligentes**

---

## 🚀 Sobre Mim

Sou um profissional apaixonado por **dados, tecnologia e automação**, com forte atuação na transformação de informações em **insights estratégicos** que apoiam a tomada de decisão e a evolução dos negócios.

Tenho facilidade em conectar **áreas técnicas e regras de negócio**, atuando desde a modelagem dos dados até a entrega de dashboards, automações e integrações entre sistemas.

- 🎓 Formado em **Análise e Desenvolvimento de Sistemas**
- 📊 Especializando-me em **Análise de Dados** pela **Data Science Academy**
- 🔄 Experiência sólida com **integrações, automação de processos e sistemas corporativos**
- 🧠 Perfil analítico, lógico e orientado à resolução de problemas

> *“Dados são apenas números até que você os transforme em histórias que impulsionam ações.”*

---

## 🧠 Soft Skills & Mindset

**Soft Skills:**  
💬 Comunicação clara • 🔥 Resiliência • 😊 Simpatia • ⚖️ Imparcialidade • 🚀 Alta performance sob pressão  

**Mindset:**  
🧩 Pensamento analítico • 📐 Lógica apurada • 💡 Pensamento crítico • 🎯 Foco em soluções escaláveis

---

## 🛠️ Tecnologias & Competências Técnicas

### 📌 Linguagens, Dados & Back-end
<img align="left" alt="Python" width="30px" style="padding-right:10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/python/python-original.svg"/>
<img align="left" alt="SQL" width="30px" style="padding-right:10px;" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/sqldeveloper/sqldeveloper-original.svg"/>

<p 
    align="left"
    title="TOTVS/ADVPL"
    style="
        display: inline-block;
        font-weight: bold;
        font-size: 16px;
        color: #0A1E32; 
        padding-right: 10px;
        margin: 0;
    "
>
    💻 <strong>ADVPL / TOTVS Protheus</strong> ⚙️
</p>

<br/><br/>

- 🗄️ **SQL avançado** para consultas complexas, manipulação e extração de dados  
- 🧱 **Modelagem de dados** e design de estruturas relacionais  
- 🔗 **Integração e consumo de APIs REST**  
- 🧩 Customizações e desenvolvimento em **ERP Protheus (AdvPL)**  

---

### 📊 BI, Análise & Visualização de Dados
<img align="left" alt="Power BI" width="30px" style="padding-right:10px;" src="https://upload.wikimedia.org/wikipedia/commons/c/cf/New_Power_BI_Logo.svg"/>
<img align="left" alt="Excel" width="30px" style="padding-right:10px;" src="https://upload.wikimedia.org/wikipedia/commons/3/34/Microsoft_Office_Excel_%282019–present%29.svg"/>

<br/><br/>

- 📈 Desenvolvimento de **dashboards interativos e relatórios gerenciais**
- 🧮 Criação de métricas, KPIs e análises avançadas com **DAX**
- ✅ Processos de **validação, consistência e garantia da qualidade dos dados**

---

### 🔄 Automação, Integrações & Infraestrutura
- 🤖 **N8N** para automação de fluxos e integrações entre sistemas
- 🐳 **Docker** para ambientes conteinerizados
- 📦 **Portainer.io** para gerenciamento, padronização e deploy de aplicações

---

### 🌐 Desenvolvimento Web
- 🧩 **Django** para desenvolvimento de aplicações web
- 🎨 **HTML, CSS e JavaScript** para interfaces funcionais e bem estruturadas
- ⚙️ Foco em soluções **escaláveis, organizadas e orientadas a dados**

---

## 📫 Como Me Encontrar

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mateus-pereira-9434b91b3/)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mateusvoid999@gmail.com)
